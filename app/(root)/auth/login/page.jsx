    ...
    const handleGoogleSuccess = async (credentialResponse) => {
        try {
            ...
            if (searchParams.has('callback')) {
                ...
                response.data.role === 'admin' ? router.push(ADMIN_DASHBOARD) : router.push(USER_DASHBOARD)
            }
        } catch (error) {
            showToast('error', error?.response?.data?.message || error.message || 'Google authentication failed.')
        } finally {
            setLoading(false)
        }
    }
