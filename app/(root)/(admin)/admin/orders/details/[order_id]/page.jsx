                                ...
                                <div className="p-5 bg-gray-50 dark:bg-card">
                                    ...
                                    <div className="pt-3">
                                        ...
                                        <div className="flex gap-4 items-center">
                                            <ButtonLoading type="button" loading={updatingStatus} onClick={handleOrderStatus} text="Save Status" className="mt-5 cursor-pointer flex-1" />
                                            {orderStatus === 'delivered' && (
                                                <a 
                                                    href={`https://wa.me/${orderData?.phone}?text=${encodeURIComponent(`*METALVERO INVOICE*\n\nHello ${orderData.name},\nYour order *#${orderData.order_id}* has been completed!\n\n*Total Amount:* ₹${orderData.totalAmount.toLocaleString('en-IN')}\n\nView Details: ${window.location.origin}/order-details/${orderData.order_id}\n\nThank you for shopping with us!`)}`}
                                                    target="_blank"
                                                    className="mt-5 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded flex items-center gap-2 font-bold text-sm h-10 transition-colors flex-1 justify-center"
                                                >
                                                    Send Bill (WA)
                                                </a>
                                            )}
